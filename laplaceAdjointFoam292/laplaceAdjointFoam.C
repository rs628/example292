...
// calculate the L2 norm of the error_y in control variables
error_y = Foam::sqrt(
                            gSum(volField * (
        ...
        errorFile.open("error_y.csv",std::ios::app);
        errorFile << runTime.value() << "," << error << nl;
        errorFile.close();


// calculate the L2 norm of the error_p in control variables
error_p = Foam::sqrt(
                            gSum(volField * (  Foam::pow(p.internalField() - pd.internalField(), 2)   ) )
                    );
                                               
                                      
  
        errorFile.open("error_p.csv",std::ios::app);
        errorFile << runTime.value() << "," << error << nl;
        errorFile.close();
